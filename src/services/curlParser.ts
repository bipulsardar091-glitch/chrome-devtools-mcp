...
const SUPPORTED_METHODS: ReadonlySet<string> = new Set<SupportedMethod>([
  'GET',
  'POST',
  'PUT',
  'PATCH',
...
function normalizeCurlCommand(curlCommand: string): string {
...
function splitCurlCommand(curlCommand: string): string[] {
  let parts: string[];
  try {
    ...
    throw new CurlParseError(
      `Unable to parse curl command: ${(err as Error).message}`,
    );
  }
...
function parseFormPart(
  ...
  if (fieldValue.startsWith('@')) {
    ...
    try {
      ...
      throw new CurlParseError(
        `Unable to open upload file '${filePath}': ${(err as Error).message}`,
      );
    }
...
function readDataValue(value: string): string | Buffer {
  if (value.startsWith('@')) {
    ...
    try {
      ...
      throw new CurlParseError(
        `Unable to read data file '${filePath}': ${(err as Error).message}`,
      );
    }
  }
  return value;
...
export function parseCurlRequest(curlCommand: string): ParsedCurlRequest {
  ...
  const cookies: Record<string, string> = {};
  let auth: [string, string] | null = null;
  const dataParts: Array<string | Buffer> = [];
  const files: FormDataField[] = [];
  const openedFiles: OpenedFileHandle[] = [];
  ...
  if (dataParts.length > 0) {
    if (paramsFromData) {
      ...
      data = (dataParts as string[]).join('&');
    } else if (dataParts.length === 1) {
      data = dataParts[0];
    } else {
      data = Buffer.concat(
        dataParts.map((p) => (Buffer.isBuffer(p) ? p : Buffer.from(String(p)))),
      );
    }
...
// Polyfill for String.prototype.partition (Python-style split into 3 parts)
declare global {
  interface String {
